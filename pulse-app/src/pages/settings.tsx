...
export function SettingsPage() {
  ...
  function addFormat() {
    ...
    add("/formats", { name, pillarId, postType: pt }, "Format");
  }
  ...
  const fmts = taxonomy.formats.filter((f) => f.pillar_id === activePillar);

  return (
    <section className="screen">
      <div className="sectitle"><span className="dot" />Content taxonomy<span className="s">drives the entry form's cascading fields</span></div>
      <div className="card pad">
        ...
        <label className="f" style={{ marginTop: 18 }}>Content Types &amp; Formats — per pillar</label>
        <select className="t" style={{ maxWidth: 320, marginBottom: 12 }} value={activePillar} onChange={(e) => setPillarId(e.target.value)}>
          {taxonomy.pillars.map((p) => (
        ...
        <div className="grid g2">
          <div>
            <div className="hint" style={{ margin: "0 0 7px", color: "var(--muted)", fontWeight: 700 }}>Content Types</div>
            <div className="taxrow">
              {cts.map((c) => (
          ...
          <div>
            <div className="hint" style={{ margin: "0 0 7px", color: "var(--muted)", fontWeight: 700 }}>Formats</div>
            <div className="taxrow">
              {fmts.map((f) => (
                <span className="taxchip" key={f.id}>
                  {f.name}
                  <span className={"ptbadge " + (f.post_type === "reel" ? "pt-reel" : "pt-car")} style={{ fontSize: 10 }}>
                    {f.post_type}
                  </span>
                  {isAdmin && (
        ...
        <div className="hint" style={{ marginTop: 10 }}>
          {isAdmin
            ? "Each pillar has its own Content Types & Formats. Deleting an item in use by posts is blocked — reassign those posts first."
